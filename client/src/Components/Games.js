...
function Games() {
  ...
  useEffect(() => {
    const fetchAllGames  = async ()=>{
      ...
      try{
        const response = await fetch("https://www.store-server.shop/games");
      ...
      //add the game to the user's cart by sending a post request
        ...
        const addToCart = async () => {
          try {
            ...
            const response = await fetch('https://www.store-server.shop/cart', {
              method: 'POST',
              headers: {
    ...
    <div>
        ...
        {currentGames.map((game, index) => (
          <Game
            key={index}
            gameName={game.Name}
            imageSource={`https://www.store-server.shop/Images/${game.ImageName}`}
            price={game.Price}
